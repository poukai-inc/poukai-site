...
- **Open product decisions for the spec** (flagged): does booking *replace* or *sit beside* `mailto:` at each conversion point (PM lean: **beside, with `mailto:` primary and booking secondary** — preserves the low-friction path while adding the high-intent one; never force a calendar on someone not ready); does the `cal.pouk.ai` link carry context (archetype/rung) the way `mailto:?subject=` does (PM lean: defer to v2 — Cal.com event-type routing is an infra question, not a launch blocker); link treatment (PM lean: a plain `<a href="https://cal.pouk.ai">` — no embed, no widget, no island — as a brand-restraint design choice). [The earlier "zero-JS posture … per R-009" framing is superseded by D-25 (2026-06-16 JS revocation) — client JS is permitted; the plain link stands as a design choice, not a contract. a11y + reduced-motion remain binding.]
...
- **P3.4 — Build** (`pouk-ai-engineer`): plain `<a href="https://cal.pouk.ai">` at the governed conversion points (plain link by design choice; the former "zero-JS" requirement is superseded by D-25 — client JS is permitted); CI route coverage already exists for the affected pages.
...
- It does not change the binding a11y (WCAG AA, axe-core 0 violations), `prefers-reduced-motion`, or security standards — every new surface inherits the existing R-NNN bar for those. [The zero-JS standard is revoked and Lighthouse/HTML-weight are advisory per D-25 (2026-06-16 JS revocation); a11y + reduced-motion + security remain binding.]
