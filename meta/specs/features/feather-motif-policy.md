...
- **Weight**: a small inline feather is byte-cheap and must not threaten R-015 (the eagle, if it ever lands, is ≤8KB gzip and is measured against R-015 before merge — but the eagle is out of this policy's build scope).
...
- [ ] **Every deliberate feather is inline `currentColor` SVG, `aria-hidden="true"`, static (no JS, no motion).** Verifier: built-HTML grep — feather SVGs are inline, carry `aria-hidden`, carry no animation; no `client:*`/`<script>` introduced (R-079).
...
- [ ] No Lighthouse / weight regression from any feather mark (R-013/R-015). Verifier: lighthouse-ci + weight on preview.
