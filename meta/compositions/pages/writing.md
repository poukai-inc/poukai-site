...
- **A2 — The ungated email line is a plain `<form action method="post">` with a bare `<Input type="email">` atom + `<Button type="submit">` — NOT the DS `<Form>` molecule.** Note: the DS `<Form>` molecule (`llms-full.txt` §Form) *requires* an `onSubmit` handler and *prevents default browser submit* — it is a hydrated JS component (spec §8, §7(d)). The composition's chosen path is a native `<form action="<buttondown-url>" method="post">` posting directly to the hosted provider, composed from the static-safe `<Input>` and `<Button>` atoms (both render as plain HTML), because a native form is the simplest, most robust path here — not because client JS is barred. See §2 Index-Section-4 and §6.
> Superseded by D-25 (2026-06-16 JS revocation): client JS permitted; Lighthouse/HTML-weight advisory. a11y + reduced-motion remain binding. The native-form choice is now a design preference; `<Form>` (hydrated) is also permitted if a future requirement wants client-side validation.
...
- **Brand notes**: "**No list, no pitch — just the notes.**" (draft §4) is the load-bearing anti-funnel line — without it, even an ungated form reads as a newsletter signup. Button label `Subscribe` is plain and honest (draft §4/§7 Q5); the line above carries the anti-funnel register so the button can stay plain. **Zero-gate by design**: native `<form action>`, no popup, no modal, no interstitial (spec §8, §7(d)) — the restraint is the no-gate posture, chosen for the reader, not a JS prohibition. The visually-hidden `<label>` gives the input an accessible name without visible chrome (the DS ships `<VisuallyHidden>`; use it or a `.visually-hidden` utility). `Input` uses `--bg-elevated` (#FFFFFF) fill by DS contract — the one permitted white surface, correct here as a form field rises above the page.
...
- **Fires on scroll**: nothing. No intersection-triggered reveal on the essay list or on stat blocks. A scroll reveal on stat blocks is omitted because ~~it would require `IntersectionObserver` = JS, breaking the zero-JS AC (spec §8)~~ — and because stat blocks must be screenshot-able at any scroll position, which a fade-in-on-intersection would fight. **The reveal does not earn its place** (a composition judgement).
- **Fires never (locked out by this composition)**: scroll reveal, parallax, scroll-spy, view counters, reading-progress bars, social-share-button widgets, comment/reaction state, animated stat counters (`NumberFormat` has "no animated counters" by DS contract anyway). ~~any `client:*` directive, any hydrated email form~~ — these are left out by design, not by a JS contract.
> Superseded by D-25 (2026-06-16 JS revocation): client JS permitted; Lighthouse/HTML-weight advisory. a11y + reduced-motion remain binding. The motion/JS lock-outs here are composition choices, no longer enforced by a zero-JS contract.
...
- **The ungated email line uses a native `<form action>`, not the DS `<Form>` molecule (composition preference).** `<Form>` (`llms-full.txt` §Form) *requires* an `onSubmit: (data) => void | Promise<void>` handler and *prevents default browser submit* — it is a hydrated JS component by contract. The composition's chosen path is a **native `<form action method="post">`** posting directly to Buttondown, composed from the static-safe `<Input>` and `<Button>` atoms — the simplest, most robust path for a no-frills subscribe line. This is a *composition* decision, not a DS gap — the right primitives exist. ~~Using `<Form>` would force a `client:*` directive and break the zero-JS AC~~ — if a future requirement needs client-side validation/feedback on the email line, the hydrated `<Form>` molecule is now permitted; choosing the native form here is a design call.
> Superseded by D-25 (2026-06-16 JS revocation): client JS permitted; Lighthouse/HTML-weight advisory. a11y + reduced-motion remain binding.
