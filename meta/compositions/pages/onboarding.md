...
- **Fires on scroll**: nothing. No intersection-triggered reveal, no parallax, no scroll-spy. An intersection reveal on the four phases is omitted because (a) ~~it would require `IntersectionObserver` = JS, breaking the zero-JS AC (spec §8)~~, and (b) it would animate the arc in a way that competes with the reader's own pace. The arc is expressed through static layout (the numbered index + the `<FailureMode>` sequence), not motion. **The reveal does not earn its place** — a composition judgement, explicit per the template requirement.
...
- **A scheduling embed / contact form / iframe / modal.** The end CTA is `mailto:` + a plain `<a>` to `cal.pouk.ai` (booking-affordance note); a plain-anchor affordance by composition choice, no widget (spec §10; R-009).
> Superseded by D-25 (2026-06-16 JS revocation): client JS permitted; Lighthouse/HTML-weight advisory. a11y + reduced-motion remain binding. (No-widget remains a composition preference, not a JS contract.)
