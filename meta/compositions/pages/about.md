...
**DS version targeted (original)**: `@poukai-inc/ui@0.14.0` (bump 2026-05-18; six Rank-1 DS-gap proposals — see §6. Five accepted in some form by 0.14.0; one partially-shipped with brand-contract counter-proposal — see §6.3 resolution).
**DS version shipped (current, 2026-05-19)**: `@poukai-inc/ui@0.15.0`. The 0.15.0 bump shipped the `<Portrait>` molecule (§6.6) — `/about` v2 is its first and currently only consumer. All six §6 Rank-1 proposals have resolved on the DS side; `/about` v2 ships against `0.15.0` as the live recipe.
