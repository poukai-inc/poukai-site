...
> **DS FINDING — the DS `FAQSection` / `FAQItem` / `Accordion` require hydration; this composition uses `Disclosure` instead (now a design preference, not a contract requirement).** Per `meta/ds-snapshot/llms-full.txt`: `Accordion` is "built on `@radix-ui/react-accordion`," and both `FAQItem` and `FAQSection` "depend on the `Accordion` molecule (`@radix-ui/react-accordion`)" — they wire `aria-expanded`/`aria-controls`/`role="region"` and the keyboard model through Radix, a React runtime that must be hydrated (`client:*`). **`<Disclosure>` is the simpler, no-hydration primitive** — its root is a **native `<details>`** with native `<summary>` semantics, "No manual ARIA needed," and chevron rotation via CSS `transition` (reduced-motion handled by the global token clamp). **This composition builds the FAQ from `<Disclosure>` rows inside a site-side `<Section>`-register wrapper — preferring the native `<details>` path for simplicity and robustness; `FAQSection` (hydrated) is also permitted now.** See §6 for the reasoning.
> Superseded by D-25 (2026-06-16 JS revocation): client JS permitted — the Radix-hydrated `FAQSection`/`FAQItem`/`Accordion` are no longer barred by a zero-JS contract (R-079/R-009 client-JS clauses revoked). Lighthouse/HTML-weight advisory. a11y + reduced-motion remain binding, so any hydrated accordion must still be keyboard-accessible, axe-clean, and reduced-motion-correct. The `<Disclosure>` choice stands as the designer's recommended default, not a mandate.
...
**One soft, optional gap — surfaced, not blocking. Both a native-`<details>` path and the hydrated `FAQSection` are now available.**

> Superseded by D-25 (2026-06-16 JS revocation): client JS permitted; Lighthouse/HTML-weight advisory. a11y + reduced-motion remain binding. The framing below ("unusable as shipped", "standards forbid", "the gap") was written under the now-revoked zero-JS contract; the Radix `FAQSection` is no longer barred. The native-`<details>` recommendation is retained as a design preference.

**The (former) gap**: the DS's *section-framed FAQ organism* (`FAQSection`) and its row (`FAQItem`) are **Radix-hydration-bound** (`@radix-ui/react-accordion`). ~~On a zero-JS site (R-009 / R-079), they are unusable as shipped — adopting them would force a `client:*` directive, which the standards forbid.~~ Under D-25 they are now permitted; the choice between them and native `<details>` is a design/complexity trade-off, with a11y + reduced-motion binding either way.

**The path this composition recommends**: compose the FAQ from `<Disclosure>` (native `<details>`) inside a site-side `<section aria-labelledby>` frame. This delivers the same reader outcome (named region, collapsible Q&A, native a11y, CSS-only chevron) without the Radix engine — preferred for its simplicity. The only thing lost is JS-enforced "single open at a time," which is not wanted anyway (A3). **This is a complete solution — nothing blocks the FAQ.**

**The DS-gap proposal (optional, Arian's call to route)**: a **`DisclosureGroup` / native-`<details>` `FAQSection` variant** — a section-framed organism that wraps `<Disclosure>` rows (native `<details>`) the way `FAQSection` wraps `FAQItem`, giving consumers the landmark + heading + lede frame without pulling in Radix. Framed from the composition need: *"a marketing site wants the `FAQSection` ergonomics (Section frame + collapsible Q&A list) with a simpler native-`<details>` engine; the existing `<Disclosure>` already proves native `<details>` is the DS's lightweight disclosure primitive — wrap it in a Section frame."* This is a convenience that would let the site drop its hand-rolled section wrapper; it is **not** required to ship the FAQ. If Arian wants it, the draft lands in `meta/proposals/ds-side/native-details-faq-section.md` describing the gap (not the API). **Recommendation: ship the `<Disclosure>` composition now; file the proposal only if a second consumer makes the hand-rolled frame annoying.** Drafting the DS API is out of this composition's lane.
...
- **`FAQSection` / `FAQItem` / `Accordion` adoption.** Not adopted here by composition preference (the native `<Disclosure>` path is simpler). ~~Refused on this site for the zero-JS reason above.~~ Under D-25 the hydrated DS organisms are permitted; choosing native `<details>` is a design call, and any hydrated accordion must still be keyboard-accessible, axe-clean, and reduced-motion-correct.
> Superseded by D-25 (2026-06-16 JS revocation): client JS permitted; Lighthouse/HTML-weight advisory. a11y + reduced-motion remain binding. (No R-009 revision is needed — the client-JS clauses are already revoked.)
